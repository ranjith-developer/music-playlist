...
class MusicPlaylist extends Component {
  ...
  renderNoSongsFoundView = () => (
    <div className="no-songs-container">
      <p className="no-songs-txt">No Songs Found</p>
    </div>
  )

  render() {
    const {playList, searchInput} = this.state
    const searchResult = playList.filter(track =>
      track.name.toLowerCase().includes(searchInput.toLowerCase()),
    )

    return (
      <div className="app-container">
        <div className="singer-details" data-testid="artist-details">
          <h1 className="singer-name">Ed Sheeran</h1>
          <p className="singer-tag">Singer</p>
        </div>
        <div className="input-container">
          <h3 className="playlist">Songs PlayList</h3>
          <input
            className="input"
            type="search"
            value={searchInput}
